...
- **🧠 Gemini 智能分析**:
  - **盘前战备 (Morning Brief)**: 08:30 运行，扫描隔夜外盘（美股/大宗/美债），推演对 A 股持仓的影响，给出开盘策略。
...
#### 0. 盘前战备 (Morning Brief)

建议在 **08:30** 左右运行，扫描隔夜外盘变动。

```bash
python3 src/main.py --mode morning
```
...
# 0. 盘前战备 (08:30)
30 8 * * 1-5 cd /root/ai_sentiney && /root/ai_sentiney/.venv/bin/python3 src/main.py --mode morning >> logs/cron.log 2>&1
...
本项目支持 **三种主要运行模式** 和 **WebUI** 界面。

### 0. 盘前战备 (Morning Brief)

建议在 **08:30** 左右运行，扫描隔夜全球市场变动。

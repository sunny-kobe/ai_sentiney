...
- **功能点 1: 实时行情 (Stability Enhanced)**
  - 实现形态: 优先批量实时行情，失败后回退到单票实时行情。
  - **稳定性要求**:
    - 多源优先级：`Tencent -> Efinance -> AkShare`
    - 批量行情失败后，继续按标的补拉单票行情
    - 单票实时行情若瞬时失败，追加一次短重试，减少整份盘中报告被误降级
  - **展示要求**:
    - 只有 `quote_status == fresh` 的标的才允许展示盘中价格和涨跌幅
    - 若实时行情缺失但历史日线仍可用，系统应输出“技术结构型降级摘要”，而不是伪造 `0.0%`
  - _目的_: 获取当前可验证的实时价格，并把“有行情”和“无行情”明确区分开。
- **功能点 2: 舆情与资金侦测**
...
| **数据过期** | 接口返回了昨日缓存数据        | 增加 `Data Integrity Check`，校验数据时间戳是否为今天，否则报警；盘中仅在 `quote_status=fresh` 时展示实时涨跌。 |
| **接口超时** | 午间高峰期实时行情瞬时失败    | 使用多源 fallback；批量失败后回补单票行情；单票行情额外短重试一次；无法确认时降级展示而不是输出假 `0.0%`。 |
